...
export default function OnboardingPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      window.open(`mailto:zenoscale@gmail.com?subject=${encodeURIComponent(`New Onboarding: ${data.businessLegalName}`)}&body=${encodeURIComponent(emailBody)}`, '_blank')
      localStorage.removeItem(STORAGE_KEY); localStorage.removeItem(STORAGE_STEP_KEY); setSubmitted(true)
...
function formatSubmissionEmail(data: OnboardingData): string {
  const lines: string[] = [
    '===================================',
    ...
    `Display Name: ${data.businessDisplayName || 'N/A'}`,
    ...
    `Years: ${data.yearsInBusiness || 'N/A'}`,
    `Brand Colors: ${data.brandColors || 'N/A'}`,
    '',
    '--- CONTACT & TEAM ---',
    `Owner: ${data.ownerName}`,
    `Owner Email: ${data.ownerEmail}`,
    `Owner Cell: ${data.ownerCell}`,
    `Lead Notify: ${data.leadNotifyName} / ${data.leadNotifyEmail} / ${data.leadNotifyPhone}`,
    `Team Access: ${data.teamNeedsAccess}`,
    `Team Members: ${data.teamMembers || 'N/A'}`,
    `Handles Calls: ${data.whoHandlesCalls || 'N/A'}`,
    `Handles Booking: ${data.whoHandlesBooking || 'N/A'}`,
    `Team Size: ${data.teamSize || 'N/A'}`,
    '',
    '--- CURRENT LEAD FLOW ---',
    `Lead Sources: ${data.leadSources}`,
    `Lead Process: ${data.leadProcess || 'N/A'}`,
    `Missed Call: ${data.missedCallProcess || 'N/A'}`,
    `Response Time: ${data.responseTime || 'N/A'}`,
    `Follow-up: ${data.followUpProcess || 'N/A'}`,
    `Sales Process: ${data.salesProcess || 'N/A'}`,
    `Avg Deal: ${data.averageDealValue || 'N/A'}`,
    `Monthly Leads: ${data.monthlyLeadVolume || 'N/A'}`,
    '',
    '--- SERVICES & OFFERS ---',
    `Services/Pricing: ${data.servicesWithPricing}`,
    `Free Consults: ${data.offersConsultations || 'N/A'}`,
    `Promotions: ${data.currentPromotions || 'N/A'}`,
    `Most Popular: ${data.mostPopularService || 'N/A'}`,
    `Highest Margin: ${data.highestMarginService || 'N/A'}`,
    `Financing: ${data.offersFinancing || 'N/A'} ${data.financingDetails || ''}`,
    '',
    '--- APPOINTMENTS ---',
    `Type: ${data.appointmentType || 'N/A'}`,
    `Appointment Types: ${data.appointmentTypes || 'N/A'}`,
    `Booking Hours: ${data.bookingDaysHours || 'N/A'}`,
    `Advance: ${data.bookingAdvance || 'N/A'}`,
    `Min Notice: ${data.minimumNotice || 'N/A'}`,
    `Buffer: ${data.bufferTime || 'N/A'}`,
    `Confirmation: ${data.confirmationMethod || 'N/A'}`,
    `Reminders: ${data.reminderTiming || 'N/A'}`,
    `Cancel Policy: ${data.cancellationPolicy || 'N/A'}`,
    '',
    '--- PHONE / SMS / EMAIL ---',
    `Phone Setup: ${data.phoneSetup || 'N/A'}`,
    `Port Provider: ${data.portProvider || 'N/A'}`,
    `Area Code: ${data.preferredAreaCode || 'N/A'}`,
    `Texting: ${data.currentlyTexting || 'N/A'}`,
    `Email List: ${data.hasEmailList || 'N/A'}`,
    `Outbound Email: ${data.outboundEmailAddress || 'N/A'}`,
    `Custom Domain: ${data.hasCustomDomain || 'N/A'} ${data.customDomain || ''}`,
    '',
    '--- REPUTATION & REVIEWS ---',
    `Google Business: ${data.googleBusinessLink}`,
    `GBP Email: ${data.googleBusinessEmail || 'N/A'}`,
    `Facebook: ${data.facebookPageUrl || 'N/A'}`,
    `Yelp: ${data.yelpUrl || 'N/A'}`,
    `Other Platforms: ${data.otherReviewPlatforms || 'N/A'}`,
    `Review Process: ${data.currentReviewProcess || 'N/A'}`,
    `Auto Reviews: ${data.wantsAutoReviews || 'N/A'}`,
    `Review Timing: ${data.reviewRequestTiming || 'N/A'}`,
    '',
    '--- ADVERTISING ---',
    `Google Ads: ${data.runningGoogleAds || 'N/A'} ${data.googleAdsId || ''}`,
    `Facebook Ads: ${data.runningFacebookAds || 'N/A'} ${data.facebookAdAccountId || ''}`,
    `Pixel: ${data.facebookPixelId || 'N/A'}`,
    `GA4: ${data.googleAnalyticsId || 'N/A'}`,
    `Other Ads: ${data.otherAdPlatforms || 'N/A'}`,
    `Monthly Budget: ${data.monthlyAdBudget || 'N/A'}`,
    `Target CPL: ${data.targetCostPerLead || 'N/A'}`,
    '',
    '--- INTEGRATIONS ---',
    `CRM: ${data.currentCrm || 'N/A'} ${data.crmOther || ''}`,
    `Data Migration: ${data.needsDataMigration || 'N/A'}`,
    `Accounting: ${data.accountingSoftware || 'N/A'}`,
    `Payments: ${data.paymentProcessor || 'N/A'} ${data.paymentEmail || ''}`,
    `Other Tools: ${data.otherTools || 'N/A'}`,
    `Zapier/Make: ${data.hasZapierMake || 'N/A'}`,
    '',
    '--- AUTOMATIONS WANTED ---',
  ]
  ...
  const automations = [
    ...
    ['Lead Nurture', data.wantLeadNurture],
    ...
    ['Re-Engagement', data.wantReEngagement],
  ...
  for (const [label, val] of automations) {
    if (val) lines.push(`  [x] ${label}`)
  }
  if (data.automationsOther) lines.push(`  Other: ${data.automationsOther}`)

  lines.push('', '===================================', '  Submitted via Zeno Onboarding', '===================================')
